...
export const ProductInfo = () => {
  ...
  return (
    <div>
      ...
      {item.product && (<h1>Name: {item.product.product_name}</h1>)}
      {item.product && (<h1>Name: {item.product.generic_name}</h1>)}
      {item.product && (<h1>Brand: {item.product.brands}</h1>)}
      {item.product && (<h1>ingredients: {item.product.ingredients_text}</h1>)}
